...
// Parser alternativo para la pagina HTML de ECU 911 si no se usa el servicio JSON.
// Normaliza texto para comparar provincias, estados y nombres de vias.
...
// Limpia una celda extraida desde tabla o texto plano.
function cleanCell(text = "") {
  return text.replace(/\s+/g, " ").trim();
}

// Convierte textos de estado a los valores que entiende el frontend.
...
// Obtiene la referencia E-* dentro del nombre de una via.
function extractRef(via = "") {
  const m = via.match(/\b(E[-\s]?\d+)\b/i);
  return m ? m[1].replace(/\s+/g, "").toUpperCase() : "";
}

// Arma el objeto incidente con el formato comun de la aplicacion.
...
// Primer intento: extraer filas desde tablas HTML tradicionales.
...
// Fallback: leer contenido plano separado por tabs o espacios multiples.
...
// Descarga la pagina HTML de ECU 911 y aplica los parsers disponibles.
