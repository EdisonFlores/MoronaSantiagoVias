...
function validateCreatePayload(payload) {
  ...
  if (tipo && !INCIDENT_TYPES[tipo] && !tipoTexto) errors.push("tipoTexto es obligatorio cuando tipo no esta en el catalogo.");
  if (!descripcion) errors.push("descripcion es obligatoria.");
  if (descripcion.length > MAX_DESCRIPTION_LENGTH) errors.push(`descripcion no debe superar ${MAX_DESCRIPTION_LENGTH} caracteres.`);
  if (!provincia) errors.push("provincia es obligatoria.");
  if (!canton) errors.push("canton es obligatorio.");
  ...
  if (!isValidLocation(location)) errors.push("ubicacion debe ser un punto valido con lat y lng.");
...
export function buildUserIncidentDocument(payload) {
  ...
  if (!validation.ok) {
    const error = new Error("Reporte ciudadano invalido.");
...
export function requireAdminKey(req) {
  ...
  if (!adminKey) {
    const error = new Error("USER_INCIDENTS_ADMIN_KEY no esta configurada.");
...
export function handleApiError(res, error) {
  if (error?.code === 5) {
    sendJson(res, 503, {
      ok: false,
      message: "Firestore no encontro la base de datos configurada. Revisa FIRESTORE_DATABASE_ID; si usas la base normal de Firebase debe ser (default)."
    });
    return;
  }
  ...
  if (error?.code === 8 || error?.code === "resource-exhausted") {
    sendJson(res, 429, {
      ok: false,
      message: "El servicio de reportes ciudadanos alcanzo temporalmente su limite de uso. Intenta nuevamente mas tarde."
    });
    return;
  }
  ...
  if (error?.code === 4 || error?.code === 14 || error?.code === "deadline-exceeded" || error?.code === "unavailable") {
    sendJson(res, 503, {
      ok: false,
      message: "El servicio de reportes ciudadanos no esta disponible temporalmente. Intenta nuevamente mas tarde."
    });
    return;
  }
  ...
  if (error?.code === 9 || error?.code === "failed-precondition") {
    sendJson(res, 503, {
      ok: false,
      message: "Firestore necesita preparar un indice para esta consulta de reportes ciudadanos."
    });
    return;
  }
