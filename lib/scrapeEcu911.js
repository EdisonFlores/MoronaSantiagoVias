// Consulta el servicio JSON de ECU 911 y normaliza vias nacionales.
...
function mapEcu911Road(item, idx, hasEcu911News = false) {
...
// Descarga y fusiona vias nacionales desde ECU 911.
export async function scrapeEcu911Ecuador() {
  ...
  const allItems = allRows.map((item, idx) => mapEcu911Road(item, idx, false));
  const newsItems = newsRows.map((item, idx) => mapEcu911Road(item, idx, true));
  ...
  console.log(
    `ECU 911 JSON devolvio ${incidents.length} vias nacionales ` +
...
// Mantiene compatibilidad con flujos antiguos que solo requieren Morona Santiago.
export async function scrapeEcu911MoronaSantiago() {
  const items = await scrapeEcu911Ecuador();

  return items.filter((item) => normalizeText(item.provincia) === "morona santiago");
}
